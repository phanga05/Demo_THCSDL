...
GO
DROP PROC IF EXISTS ThemMatHang;
GO
...
BEGIN
    ...
    BEGIN
        ...
        RETURN;
    END
    IF NOT EXISTS (SELECT 1 FROM MATHANG WHERE MaCongTyNo = @MaCongTyNo)
    BEGIN
		RAISERROR('Mã công ty không tồn tại.', 16, 1);
        RETURN;
    END

    IF NOT EXISTS (SELECT 1 FROM MATHANG WHERE MaLoaiHangNo = @MaLoaiHangNo)
    BEGIN
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM MATHANG m
        INNER JOIN inserted i ON m.MaHang = i.MaHangNo
        INNER JOIN deleted d ON m.MaHang = d.MaHangNo
        WHERE (i.SoLuong + d.SoLuong) > m.SoLuong OR i.SoLuong < 1
    )
    BEGIN
        PRINT N'Số lượng hiện có trong MATHANG không đủ hoặc SoLuong đặt nhỏ hơn 1'
	...
	BEGIN
		UPDATE MH
		SET MH.SoLuong = MH.SoLuong + d.SoLuong
		FROM MATHANG MH
...
UPDATE CHITIETDATHANG
SET SoLuong=20
WHERE MaHangNo='MH00000010' AND SoHoaDonNo='DH00000018'